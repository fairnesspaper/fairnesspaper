        ...
        for step, train_batch in enumerate(tqdm(train_loader)):
            ...
            if args.task == 'ranking':
                ...
                if args.experiment == 'penalty_neg' or args.experiment == 'penalty_both':
                    ...
                    # Ensure regularizer_term is initialized

                    # Compute regularizer_term based on provided inputs
           
                    regularizer_term = args.regularizer_neg * (1 - train_batch['label'].to(device)) * train_batch['bias'].to(device)
                    # Add regularizer_term to batch_score and apply sigmoid to ensure it's in [0, 1]
                    # print(batch_score)
                    # print(torch.sigmoid(batch_score))
