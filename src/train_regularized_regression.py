...
from OpenMatch.models.my_bert_pointwise import BertPointwise
        ...
        for step, train_batch in enumerate(tqdm(train_loader)):
            ...
            if args.task == 'ranking':
                ...
                if args.experiment == 'penalty_neg': # or args.experiment == 'penalty_both':
                    regularizer_term = args.regularizer_neg * (1 - train_batch['label'].to(device)) * train_batch['bias'].to(device)
                    # batch_score = batch_score.sigmoid() + regularizer_term
                    batch_score = torch.sigmoid(batch_score+regularizer_term)

                elif args.experiment == 'penalty_pos': #or args.experiment == 'penalty_both':
                    regularizer_term = args.regularizer_neg * (train_batch['label'].to(device)) * train_batch['bias'].to(device)
                    # batch_score = batch_score.sigmoid() + regularizer_term
                    batch_score = torch.sigmoid(batch_score+regularizer_term)

                elif args.experiment == 'penalty_both':
                    regularizer_term = args.regularizer_neg *  train_batch['bias'].to(device)
                    batch_score = torch.sigmoid(batch_score+regularizer_term)
