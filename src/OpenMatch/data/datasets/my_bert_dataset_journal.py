...
class BertDataset(Dataset):
    def __init__(
        ...
        if isinstance(self._dataset, str):
            ...
            with open(self._dataset, 'r') as f:
                ...
                for i, line in enumerate(f):
                    if i >= self._max_input:
                        break
                    if self._mode != 'train' or self._dataset.split('.')[-1] == 'json' or self._dataset.split('.')[
                        -1] == 'jsonl':
                        ...
                        if self._task == 'ranking':
                            ...
                            line = {'query': query, 'doc_pos': doc_pos, 'doc_neg': doc_neg, 
                                        'bias_pos': bias_pos, 'bias_neg': bias_neg, 
                                        'fairness_pos': fairness_pos, 'fairness_neg': fairness_neg}
                        elif self._task == 'classification':
                            query, doc, label, bias, fairness = line.strip('\n').split('\t')
                            line = {'query': query, 'doc': doc, 'label': int(label), 
                            'bias': float(bias), 'fairness': float(fairness)}
                        else:
            ...
            with open(self._dataset['queries'], 'r') as f:
                for line in f:
                    if self._dataset['queries'].split('.')[-1] == 'json' or self._dataset['queries'].split('.')[
                        -1] == 'jsonl':
                        line = json.loads(line)
                    else:
            ...
            with open(self._dataset['docs'], 'r') as f:
                for line in f:
                    if self._dataset['docs'].split('.')[-1] == 'json' or self._dataset['docs'].split('.')[
                        -1] == 'jsonl':
                        line = json.loads(line)
                    else:
                ...
                for i, line in enumerate(f):
                    ...
                    if self._mode == 'train':
                        ...
                        self._examples.append(
                            {'label': label, 'query_id': line[0], 'doc_id': line[2], 'retrieval_score': float(line[4])})
                    elif self._mode == 'test':
                        self._examples.append(
                            {'query_id': line[0], 'doc_id': line[2], 'retrieval_score': float(line[4])})
                    else:
    ...
    def collate(self, batch: Dict[str, Any]):
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                bias_neg = torch.tensor([float(item['bias_neg']) for item in batch])
                bias_pos = torch.tensor([float(item['bias_pos']) for item in batch])
                fairness_neg = torch.tensor([float(item['fairness_neg']) for item in batch])
                fairness_pos = torch.tensor([float(item['fairness_pos']) for item in batch])

                return {'input_ids_pos': input_ids_pos, 'segment_ids_pos': segment_ids_pos,
                        'input_mask_pos': input_mask_pos,
                        'input_ids_neg': input_ids_neg, 'segment_ids_neg': segment_ids_neg,
                        'input_mask_neg': input_mask_neg,
                        'bias_neg': bias_neg,
                        'bias_pos': bias_pos,
                        'fairness_neg': fairness_neg,
                        'fairness_pos': fairness_pos}
            elif self._task == 'classification':
                ...
                bias = torch.tensor([item['bias'] for item in batch])
                fairness = torch.tensor([item['fairness'] for item in batch])
    ...
    def pack_bert_features(self, query_tokens: List[str], doc_tokens: List[str]):
        input_tokens = [self._tokenizer.cls_token] + query_tokens + [self._tokenizer.sep_token] + doc_tokens + [
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self._mode == 'train':
            if self._task == 'ranking':
                ...
                doc_tokens_pos = self._tokenizer.tokenize(example['doc_pos'])[
                                 :self._seq_max_len - len(query_tokens) - 3]
                doc_tokens_neg = self._tokenizer.tokenize(example['doc_neg'])[
                                 :self._seq_max_len - len(query_tokens) - 3]
                ...
                bias_neg = example['bias_neg']
                bias_pos = example['bias_pos']
                fairness_neg = example['fairness_neg']
                fairness_pos = example['fairness_pos']
                ...
                return {'input_ids': input_ids, 'segment_ids': segment_ids, 'input_mask': input_mask,
                        'label': example['label'], 'bias': example['bias'], 'fairness': example['fairness']}
            else:
                raise ValueError('Task must be `ranking` or `classification`.')
        elif self._mode == 'dev':
            query_tokens = self._tokenizer.tokenize(example['query'])[:self._query_max_len]
            ...
            input_ids, input_mask, segment_ids = self.pack_bert_features(query_tokens, doc_tokens)
            return {'query_id': example['query_id'], 'doc_id': example['doc_id'], 'label': example['label'],
            ...
            input_ids, input_mask, segment_ids = self.pack_bert_features(query_tokens, doc_tokens)
            return {'query_id': example['query_id'], 'doc_id': example['doc_id'],
                    'retrieval_score': example['retrieval_score'],
                    'input_ids': input_ids, 'input_mask': input_mask, 'segment_ids': segment_ids}
        else:
            raise ValueError('Mode must be `train`, `dev` or `test`.')
